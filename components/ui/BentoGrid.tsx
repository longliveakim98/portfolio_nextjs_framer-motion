...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("mohdaimanhakim.98@gmail.com");
    setCopied(true);
  };
  return (
    <div
        ...
        {id === 3 && (
          <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
            <div className="flex flex-col gap-3 lg:gap-5">
              {["ReactJs", "React Native", "ExpressJs"].map((item, i) => (
                <span
                  key={i}
                  className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-50 hover:opacity-100 rounded-lg text-center bg-[#10132E]"
                >
                  {item}
                </span>
              ))}
            ...
            <div className="flex flex-col gap-3 lg:gap-5">
              <span className="py-2 px-3 rounded-lg text-center bg-[#10132e]" />
              {["Laravel", "TailwindCSS", "Typescript"].map((item, i) => (
                <span
                  key={i}
                  className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-50 hover:opacity-100 rounded-lg text-center bg-[#10132E]"
                >
